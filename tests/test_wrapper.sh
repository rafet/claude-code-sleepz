...
HOOK_TS=$(python3 -c "import time; print(time.time() - 120)")
...
HOOK_TS=$(python3 -c "import time; print(time.time() - 0.5)")
OUTPUT=$(bash "$WRAPPER" 1 "$HOOK_TS" 2>&1)
...
HOOK_TS=$(python3 -c "import time; print(time.time() - 0.01)")
OUTPUT=$(bash "$WRAPPER" 0.1 "$HOOK_TS" 2>&1)
assert_contains "adjusted message" "adjusted 0.1s ->" "$OUTPUT"
